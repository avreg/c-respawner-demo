...
> Андрей, в продолжение разговора с коллегами, направляю вам тестовое задание.
>
...
#### Периодически завершающее с ошибкой приложение
...
* демон делает несколько fork(2) и полностью отрывается от respawner-а, мониторим по его pidfile-у.
* нормальное завершение по TERM, как не странно, 1 (exit status), это тоже приходится учитывать.
