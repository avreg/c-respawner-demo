...
Если целевая программа (простая или демон) завершаются штатно (нормально, не аварийно), тогда завершается и сам процесс respawner.
...
```bash
# запуск простого приложение - не демона, которое "валится" по таймауту
